app.filter('string', ['$filter', ($filter) => {
    return (input) => {
        return input ? input : "NA";
    };
}]);
app.filter('isActive', ['$filter', ($filter) => {
    return (input) => {
      
        return input ? "Enabled" : "Disabled";
    };
}]); 
app.filter('yesNo', ['$filter', ($filter) => {
    return (input) => {

        return input ? "Yes" : "No";
    };
}]);
app.filter('status', ['$filter', ($filter) => {
    return (input) => {
...
app.filter('UserNameFromList', ['$filter', ($filter) => {
    return (input: any[]) => {
        var names = input.map((d) => {
            return d.firstName + " " + d.lastName;
        });
        return names.join("; ")
    };
}]);
app.filter('availableNA', ['$filter', ($filter) => {
    return (input,param) => {
        return input ? input :param || "N/A";
    };
}]);

app.filter('processMonth', ['$filter', ($filter) => {
    return (input) => {
        return [
